...
@safetestset "LQDS Tests Lower, Float64" begin
  using LinearAlgebra
  using DenseSpectralTransformation.TestLQDS
  n = 30
  x = exp.(-.2 * (1:n))
  A = Float64[
  ...
  tol = 1e-13
  TestLQDS.run_test(Hermitian(A, :L), tol)
end
...
@safetestset "LQDS Tests Lower, Float64" begin
  ...
  tol = 1e-13
  TestLQDS.run_test(Hermitian(A, :U), tol)
end
...
@safetestset "Eigenvalue/vector Test, Float64" begin
  using LinearAlgebra
  using DenseSpectralTransformation.TestSpectral
  n = 30
  x = exp.(-.2 * (1:n))
  A = Float64[
    (i == j ? (-1)^i * x[abs(i - j) + 1] : x[abs(i - j) + 1])
  ...
  tol = 1e-13
  TestSpectral.run_test(A, B, 5.0, tol)
end
